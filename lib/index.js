...
var addSpheres = function addSpheres(numSpheres, diameter) {
  for (var i = 0; i < numSpheres; i++) {
    var geometry = new _three.SphereGeometry(diameter, 32, 32);
    var sphere = new _three.Mesh(geometry, sphereMaterial);
...
var init = function init(_ref) {
  ...
  scene.fog = new _three.FogExp2(0x000000, 0.0006);

  sphereMaterial = new _three.MeshBasicMaterial({ color: new _three.Color(sphereColor) });
  spheres = new _three.Group();

  lineMaterial = new _three2.MeshLineMaterial({
    lineWidth: lineWidth,
    color: new _three.Color(lineColor)
  });
  ...
  for (var i = 0; i < spheres.children.length; i++) {
    ...
    var near = (0, _utils.getNearbyObjectsFor)(obj, spheres, maxConnectDistance);

    if (near.length == 0) continue;

    var lines = Math.floor(maxConnectLines * Math.random());
    for (var k = 0; k < near.length && k <= lines; k++) {
      var lineGeometry = new _three.Geometry();
      lineGeometry.vertices.push(new _three.Vector3(obj.position.x, obj.position.y, obj.position.z));
      lineGeometry.vertices.push(new _three.Vector3(near[k].position.x, near[k].position.y, near[k].position.z));
      var g = new _three2.MeshLine();
      g.setGeometry(lineGeometry);
      g.verticesNeedUpdate = true;
      var mesh = new _three.Mesh(g.geometry, lineMaterial);
      scene.add(mesh);
    }
  }

  camera = new _three.PerspectiveCamera(55, container.offsetWidth / container.offsetHeight, 2, 3000);
...
var render = function render() {
  ...
  if (animations.color) {
    ...
    lineMaterial.uniforms.color.value = (0, _utils.setHSL)(h, 0.5, 0.5);
  }
